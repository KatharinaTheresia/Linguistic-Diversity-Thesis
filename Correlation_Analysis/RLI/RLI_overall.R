...
df12 <- read_excel("DiGix_cleaned.xlsx") %>%
  rename_with(~ tolower(trimws(.))) 
df13 <- read_excel("RLI_codes.xlsx") %>%
  rename_with(~ tolower(trimws(.)))


# Add RLI values to all digitization dfs 
...
for (df in dfs) {
  ...
  for (yr in unique(df$year)) {
    ...
    # Compute correlations between each index column and RLI for the current year
    for (col in index_cols) {
      # Ensure there are enough observations for the correlation computation
      if (sum(!is.na(df_year[[col]]) & !is.na(df_year$rli)) > 1) {
        test <- cor.test(df_year[[col]], df_year$rli, use = "pairwise.complete.obs")
...
ggplot(heatmap_df, aes(x = Index, y = as.factor(Year), fill = Correlation)) +
  ...
  labs(title = "Correlations between RLI and Digitization Indices Over Time",
       x = "Index", y = "Year") +
  theme_minimal() +
