...
colnames(numeric_data) <- c("MCI", "DSTRI","3i", "RLI")
...
print(cor_matrix_pair)
            ...
            #MCI       DSTRI         3i        RLI
#MCI   1.0000000  0.4743299  0.9715187 -0.2171705
#DSTRI  0.4743299  1.0000000  0.4690012 -0.2674856
#3i    0.9715187  0.4690012  1.0000000 -0.2263838
#RLI  -0.2171705 -0.2674856 -0.2263838  1.0000000

          #MCI        DSTRI         3i         RLI
#MCI  1.0000000  0.46672814  0.9529828  0.13283843
#DSTRI 0.4667281  1.00000000  0.4690012 -0.02899858
#3i   0.9529828  0.46900121  1.0000000 -0.19439309
#RLI  0.1328384 -0.02899858 -0.1943931  1.00000000

# significant difference for RLI vs DSTRI

write.csv(cor_matrix, "2017_1_cor_matrix.csv", row.names = TRUE)
write.csv(cor_matrix_pair, "2017_2_cor_matrix_pair.csv", row.names = TRUE)
...
plot1 <- ggcorrplot(cor_matrix_pair, hc.order = TRUE,  
  ...
  ggtitle("Correlation Heatmap (pairwise complete obs.) 2017") +  # Add title
  theme(
...
plot2 <- ggcorrplot(cor_matrix, hc.order = TRUE,  
                    type = "lower",  
                    colors = c("#6D9EC1", "yellow", "#E46726")) +
  ggtitle("Correlation Heatmap (complete obs.) 2017") +  # Add title
  theme(
    axis.text.x = element_text(size = 8),
    axis.text.y = element_text(size = 8),
    plot.title = element_text(hjust = 0.5, size = 12, face = "bold")  # Center title
...
# Arrange plots
save_plot("2017_01_plot_heatmap.jpg", {
...
save_plot("2017_02_plot_pairs.jpg", pairs(
  ~ MCI + DSTRI + `3i` + RLI, 
  data = numeric_data, 
  lower.panel = panel.cor,
  cex.labels = 1,
  main = "Pairwise Correlations Plot with RLI (2017)" 
))
...
write.csv(summary_df, "2017_03_table_summary_comp.csv", row.names = TRUE)
...
write.csv(summary_df, "2017_04_table_summary_pair.csv", row.names = TRUE)
...
#RLI  -0.2171705 -0.2674856 -0.2263838  1.0000000
            ...
            #MCI        DSTRI         3i         RLI
#MCI   1.00000000  0.48994045  0.9715187 -0.03047757
#DSTRI  0.48994045  1.00000000  0.4690012 -0.02553474
#3i    0.97151871  0.46900121  1.0000000 -0.22638377
#RLI  -0.03047757 -0.02553474 -0.2263838  1.00000000
